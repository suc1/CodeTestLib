...
3. 考虑有一块贯通的竖缝坏B(n,m)的垒法, 左边是好的垒法, 右边是好坏全部的垒法:
设第1块砖有一块贯通的缝，G(n,1) * P(n, m-1),
设第2块砖有一块贯通的缝，G(n,2) * P(n, m-2),
...
设第m-1块砖有一块贯通的缝，G(n,m-1) * P(n, 1),

所以通式是
B(n,m) = G(n,1) * P(n, m-1) + G(n,2) * P(n, m-2) + ... + G(n,m-1) * P(n, 1)

4. 所以，好垒法:
G(n,m) = P(m)^n - B(n,m) = P(m)^n - SUM[1<=i<=m-1][G(n, i) * P(n, m-i)]
G(n,1) = 1
G(n,2) = P(2)^n - G(n,1) * p(1)^n

5. 算式里面有许多重复计算, G(n,m), P(m)

6. m是变量，n是常量。为了编程方便，m从0开始
